    ...
    public class Window : GameWindow
    {
       ...
       /* {
        ...
        protected override void OnLoad()
        {
            ...
            _texture1.Use();
            //纹理-2:
            _texture2 = new Texture("Resources/937.jpg");//awesomeface
        ...
        private void initData()
        {
           ...
           _idxSphere.CopyTo(_indices, iArrLen); 
            ...
            #region ring---使用字典装载顶点数据与索引数据,数组大小自动计算,算法紧凑.
            //顶点赋值:
            ...
            #region cube---使用字典装载顶点数据与索引数据,数组大小自动计算,算法紧凑.
            //顶点赋值:
            ...
            #region capsule---使用字典装载顶点数据与索引数据,数组大小自动计算,算法紧凑.
            //顶点赋值:
        ...
        private void dwSphere()
        {
            ...
            GL.DrawElements(PrimitiveType.Triangles, dLen["sphere"], DrawElementsType.UnsignedInt, dPtr["sphere"]);//_indices.Length
        }
        ...
        private void dwBar()
        {
            ...
            //作旋转变换:
            transform *= Matrix4.CreateTranslation((float)MathHelper.Cos(-t / 30) * 0.2f, (float)MathHelper.Sin(-t / 30) * 0.9f, -0.5f);//作平移变换
            ...
            //GL.PointSize(8); //以点集绘制时适用
            GL.LineWidth(4);//以线段集绘制时适用 
            GL.DrawElements(PrimitiveType.Triangles, dLen["bar"], DrawElementsType.UnsignedInt, dPtr["bar"]);
        }
        ...
        private void dwRing()
        {
            ...
            transform *= Matrix4.CreateScale(0.4f);
            //作旋转变换:
            transform *= Matrix4.CreateTranslation((float)MathHelper.Cos(-t / 30) * 0.7f, (float)MathHelper.Sin(-t / 30) * 0.3f, -0.5f);//作平移变换

            //依Z-Y-X顺序,绕坐标轴作旋转变换:
            //
            //transform *= Matrix4.CreateRotationY(MathHelper.DegreesToRadians(-t / .5f));
            transform *= Matrix4.CreateRotationZ(MathHelper.DegreesToRadians(-t / 3f));
            transform *= Matrix4.CreateRotationX(MathHelper.DegreesToRadians(-t / 2));
            ...
            //GL.PointSize(8); //以点集绘制时适用
            GL.LineWidth(4);//以线段集绘制时适用 
            GL.DrawElements(PrimitiveType.Triangles, dLen["ring"], DrawElementsType.UnsignedInt, dPtr["ring"]);
        }
        ...
        private void dwCube()
        {
            ...
            //作旋转变换:
            transform *= Matrix4.CreateTranslation((float)MathHelper.Cos(-t / 30) * 1.2f, (float)MathHelper.Sin(-t / 30) * 0.6f, -0.5f);//作平移变换
            ...
            //GL.PointSize(8); //以点集绘制时适用
            GL.LineWidth(4);//以线段集绘制时适用 
            GL.DrawElements(PrimitiveType.Triangles, dLen["cube"], DrawElementsType.UnsignedInt, dPtr["cube"]);
        }

        private void dwCapsule()
        {
            Matrix4 transform = Matrix4.Identity;
            //作缩放变换:
            transform *= Matrix4.CreateScale(0.4f);
            ...
            //作旋转变换:
            //transform *= Matrix4.CreateTranslation((float)MathHelper.Cos(-t / 30) * 0.2f, (float)MathHelper.Sin(-t / 30) * 0.6f, -0.5f);//作平移变换

            //依Z-Y-X顺序,绕坐标轴作旋转变换:
            //
            //transform *= Matrix4.CreateRotationY(MathHelper.DegreesToRadians(-t / .5f));
            transform *= Matrix4.CreateRotationZ(MathHelper.DegreesToRadians(-t / 2f));
            transform *= Matrix4.CreateRotationX(MathHelper.DegreesToRadians(-t / 3f));
            ...
            //GL.PointSize(8); //以点集绘制时适用
            GL.LineWidth(4);//以线段集绘制时适用 
            GL.DrawElements(PrimitiveType.Triangles, dLen["capsule"], DrawElementsType.UnsignedInt, dPtr["capsule"]);
        }
        ...
        private void dwSide6V()
        {
            ...
            //GL.PointSize(8); //以点集绘制时适用
            GL.LineWidth(4);//以线段集绘制时适用 
            GL.DrawElements(PrimitiveType.Triangles, dLen["side6V"], DrawElementsType.UnsignedInt, dPtr["side6V"]);
        }
